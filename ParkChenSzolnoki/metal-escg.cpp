...
void refreshRandomNumbers(MetalContext& ctx, float* invasion_probabilities, uint32_t* cells, uint32_t* neighbours, int N, bool moore) {
    ...
    encoder->setComputePipelineState(ctx.pipelineStateInvasions);
    encoder->setBuffer(ctx.seedBuffer, 0, 0);
    encoder->setBuffer(ctx.resultBufferInvasions, 0, 1);
    encoder->dispatchThreads(gridSize, threadGroupSize);
    encoder->endEncoding();
    commandBuffer->commit();
    commandBuffer->waitUntilCompleted();
    std::memcpy(invasion_probabilities, ctx.resultBufferInvasions->contents(), sizeof(float) * ctx.numRandomNumbers);
