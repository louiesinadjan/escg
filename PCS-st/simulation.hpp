...
float dominates(int specie, int neighbour, int speciesNum, float* dominance); // RPSLS interaction
int wrap(int index, int L);                                                   // Helper function to make the grid like a torus
void step(Params p, int* grid, float* dominance);                             // Elementary time step
