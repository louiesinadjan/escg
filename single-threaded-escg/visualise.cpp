...
void plot_snapshot(const Species grid[200][200], int L, int mcs) {
    // Create a 2D vector to hold the grid data for plotting
    ...
    // Plot the grid snapshot using imshow with appropriate arguments
    plt::figure();
    plt::imshow(reinterpret_cast<const float*>(gridData.data()), L, L, 1, {{"cmap", "jet"}});
